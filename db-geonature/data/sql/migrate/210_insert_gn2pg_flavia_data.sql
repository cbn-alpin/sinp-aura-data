...
\echo '-------------------------------------------------------------------------------'
\echo 'Batch direct insertion of the Flavia data in Synthese'
...
BEGIN
    ...
    RAISE NOTICE 'Start to loop on Flavia data to insert in "gn_synthese.synthese" table' ;
    WHILE affectedRows > 0 LOOP

        RAISE NOTICE '-------------------------------------------------' ;
        RAISE NOTICE 'Inserting % Flavia data from %', step, offsetCnt ;
