...
\echo '-------------------------------------------------------------------------------'
\echo 'Batch insertion of the LPO data in Synthese with the Gn2Pg trigger'
...
BEGIN
    ...
    SELECT COUNT(*) INTO stopAt FROM gn2pg_lpo.data_json ;
    RAISE NOTICE 'Total found: %, step used: %', stopAt, step ;

    RAISE NOTICE 'Start to loop on LPO data to trigger insert in "gn_synthese.synthese" table' ;
    WHILE offsetCnt < stopAt LOOP

        RAISE NOTICE '-------------------------------------------------' ;
        RAISE NOTICE 'Try to insert % LPO data from %', step, offsetCnt ;
        ...
        UPDATE gn2pg_lpo.data_json AS d
