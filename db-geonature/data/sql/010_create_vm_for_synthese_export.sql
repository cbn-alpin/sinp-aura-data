...
-- gn_synthese.synthese_municipality source

CREATE MATERIALIZED VIEW gn_synthese.synthese_municipality
TABLESPACE pg_default
AS SELECT cas.id_synthese,
   ...
   FROM gn_synthese.cor_area_synthese cas
     ...
     JOIN ref_geo.bib_areas_types ta ON ta.id_type = a.id_type AND ta.type_code::text = 'COM'::text
  GROUP BY cas.id_synthese
WITH DATA;

-- View indexes:
CREATE INDEX synthese_municipality_id_synthese_idx ON gn_synthese.synthese_municipality USING btree (id_synthese);
...
-- taxonomie.taxon_area_status source

CREATE MATERIALIZED VIEW taxonomie.taxon_area_status
TABLESPACE pg_default
AS WITH agg_code_status AS (
         SELECT ta.cd_ref,
            cta.id_area,
            te.cd_type_statut,
            jsonb_agg(DISTINCT bdc_statut_values.code_statut) AS code_statuts
           FROM taxonomie.bdc_statut_taxons ta
             JOIN taxonomie.bdc_statut_cor_text_values bsctv ON bsctv.id_value_text = ta.id_value_text
             JOIN taxonomie.bdc_statut_text te ON te.id_text = bsctv.id_text
             JOIN taxonomie.bdc_statut_cor_text_area cta ON cta.id_text = te.id_text
             LEFT JOIN taxonomie.bdc_statut_values ON bdc_statut_values.id_value = bsctv.id_value
...
-- View indexes:
CREATE INDEX taxon_area_status_cd_ref_id_area_idx ON taxonomie.taxon_area_status USING btree (cd_ref, id_area);
CREATE INDEX taxon_area_status_status_idx ON taxonomie.taxon_area_status USING gin (status);
...
-- gn_synthese.synthese_status source
...
AS SELECT s.id_synthese,
   ...
   FROM gn_synthese.synthese s
     ...
     JOIN taxonomie.taxon_area_status tas ON cas.id_area = tas.id_area AND t.cd_ref = tas.cd_ref
WITH DATA;

-- View indexes:
CREATE INDEX synthese_status_id_synthese_idx ON gn_synthese.synthese_status USING btree (id_synthese);
...
-- gn_synthese.v_synthese_for_export_tmp source
...
AS SELECT s.id_synthese,
   ...
   FROM gn_synthese.synthese s
     ...
     LEFT JOIN gn_synthese.synthese_municipality sm ON sm.id_synthese = s.id_synthese
     ...
     LEFT JOIN gn_synthese.synthese_status ss ON s.id_synthese = ss.id_synthese
WITH DATA;

-- View indexes:
CREATE INDEX v_synthese_for_export_id_synthese_idx ON gn_synthese.v_synthese_for_export_tmp USING btree (id_synthese);
