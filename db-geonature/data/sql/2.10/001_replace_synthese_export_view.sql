...
-- \echo '----------------------------------------------------------------------------'
-- \echo 'Create MATERIALIZED VIEW gn_synthese.v_synthese_for_export'

-- DROP MATERIALIZED VIEW IF EXISTS gn_synthese.v_synthese_for_export ;

-- CREATE MATERIALIZED VIEW gn_synthese.v_synthese_for_export
-- AS
...
--     END AS sensibilite,
--     CASE
--         WHEN ( ns.cd_nomenclature != '0' OR dl.cd_nomenclature != '5' ) THEN 'donnée confidentielle'
...
--     LEFT JOIN ref_nomenclatures.t_nomenclatures AS dl ON s.id_nomenclature_diffusion_level = dl.id_nomenclature
-- WITH DATA ;

-- CREATE UNIQUE INDEX idx_vsfe_id_synthese ON gn_synthese.v_synthese_for_export USING btree (id_synthese);
-- CREATE INDEX idx_vsfe_jdd_id ON gn_synthese.v_synthese_for_export USING btree (jdd_id);
-- CREATE INDEX idx_vsfe_id_digitiser ON gn_synthese.v_synthese_for_export USING btree (id_digitiser);
-- -- CREATE INDEX idx_vsfe_geojson_local ON gn_synthese.v_synthese_for_export USING btree (geojson_local);
-- CREATE INDEX idx_vsfe_id_nomenclature_sensitivity ON gn_synthese.v_synthese_for_export USING btree (id_nomenclature_sensitivity);
-- CREATE INDEX idx_vsfe_id_nomenclature_diffusion_level ON gn_synthese.v_synthese_for_export USING btree (id_nomenclature_diffusion_level);


\echo '----------------------------------------------------------------------------'
\echo 'Create VIEW gn_synthese.v_synthese_for_export for test and backup'
