...
-- See: https: / / github.com / PnX - SI / TaxHub / blob / 1.10.7 / apptax / taxonomie / commands / utils.py #L70
BEGIN;
...
regions_dep_areas AS (
    ...
    WHERE
        id_type = ref_geo.get_id_area_type('DEP')
),
texts AS (
    SELECT
        -- Si 'TERFXFR', 'ETATFRA' insertion de tous les départements
        bst.id_text,
        la.id_area
    FROM
    ...
    WHERE
        la.id_type = ref_geo.get_id_area_type('DEP')
...
INSERT INTO
    taxonomie.bdc_statut_cor_text_area (id_text, id_area)
SELECT
    id_text,
    id_area
FROM
