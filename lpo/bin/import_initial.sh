#!/bin/bash
# Encoding : UTF-8
# Import in GeoNature Database the LPO Fauna Data for AURA
...
Usage: ./$(basename $BASH_SOURCE)[options]
     ...
     -f | --force: force recreating RTI file for specified type:
                s (=synthese),
                so =(source),
                d (=dataset),
...
function parseScriptOptions() {
    ...
    while getopts "hvxc:f:" option; do
        case "${option}" in
            "h") printScriptUsage ;;
...
function main() {
    ...
    redirectOutput "${lpo_log_imports}"
    checkSuperuser
...
function extractArchive() {
    ...
    if [[ -f "${raw_dir}/${lpo_filename_archive}" ]]; then
        if [[ ! -f "${raw_dir}/${lpo_filename_synthese}" ]]; then
            cd "${raw_dir}/"
            tar jxvf "${raw_dir}/${lpo_filename_archive}"
        else
...
function insertOrganism() {
    local csv_to_import="${lpo_filename_organism%.csv}_rti.csv"

    printMsg "Parsing ORGANISM CSV file..."
    if [[ ! -f "${raw_dir}/${csv_to_import}" ]] || [[ "${force}" == "o" ]] ; then
        cd "${root_dir}/import-parser/"
        pipenv run python ./bin/gn_import_parser.py \
            --type "o" \
            --config "${conf_dir}/parser_actions.ini" \
            "${raw_dir}/${lpo_filename_organism}"
    else
...
function insertUser() {
    local csv_to_import="${lpo_filename_user%.csv}_rti.csv"

    printMsg "Parsing USER CSV file..."
    if [[ ! -f "${raw_dir}/${csv_to_import}" ]] || [[ "${force}" == "u" ]] ; then
        cd "${root_dir}/import-parser/"
        pipenv run python ./bin/gn_import_parser.py \
            --type "u" \
            --config "${conf_dir}/parser_actions.ini" \
            "${raw_dir}/${lpo_filename_user}"
    else
...
function insertAcquisitionFramework() {
    local csv_to_import="${lpo_filename_af%.csv}_rti.csv"

    printMsg "Parsing ACQUISITION FRAMEWORK CSV file..."
    if [[ ! -f "${raw_dir}/${csv_to_import}" ]] || [[ "${force}" == "af" ]]; then
        cd "${root_dir}/import-parser/"
        pipenv run python ./bin/gn_import_parser.py \
            --type "af" \
            --config "${conf_dir}/parser_actions.ini" \
            "${raw_dir}/${lpo_filename_af}"
    else
...
function insertDataset() {
    local csv_to_import="${lpo_filename_dataset%.csv}_rti.csv"

    printMsg "Parsing DATASET CSV file..."
    if [[ ! -f "${raw_dir}/${csv_to_import}" ]] || [[ "${force}" == "d" ]]; then
        cd "${root_dir}/import-parser/"
        pipenv run python ./bin/gn_import_parser.py \
            --type "d" \
            --config "${conf_dir}/parser_actions.ini" \
            "${raw_dir}/${lpo_filename_dataset}"
    else
...
function prepareSynthese() {
    ...
    printMsg "Restoring GeoNature database after deleting data into synthese table ..."
    export PGPASSWORD="${db_pass}"; \
        psql -h "${db_host}" -U "${db_user}" -d "${db_name}" \
...
function insertSynthese() {
    local csv_to_import="${lpo_filename_synthese%.csv}_rti.csv"

    printMsg "Parsing SYNTHESE CSV file..."
    if [[ ! -f "${raw_dir}/${csv_to_import}" ]] || [[ "${force}" == "s" ]]; then
        cd "${root_dir}/import-parser/"
        pipenv run python ./bin/gn_import_parser.py \
            --type "s" \
            --config "${conf_dir}/parser_actions.ini" \
            --report "${var_dir}/reports" \
            "${raw_dir}/${lpo_filename_synthese}"
    else
