/*
 ...
 Before to start, please adapt the name of the schema 'gn2pg_flavia' to the one you set in your config file.
 ...
 To check if you have duplicates (and delete them if wanted), see : duplicates.sql file
...
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_tri_c_upsert_data_to_geonature_with_metadata();

-- -------------------------------------------------------------------------------------------------
-- Acquisition Frameworks

DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_get_or_insert_basic_af_from_uuid_name(_uuid UUID, _name TEXT) ;

CREATE OR REPLACE
    FUNCTION gn2pg_flavia.fct_c_get_or_insert_basic_af_from_uuid_name(_uuid UUID, _name TEXT)
    RETURNS INTEGER
AS
$func$
DECLARE
...
-- -------------------------------------------------------------------------------------------------
-- Datasets
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_get_or_insert_basic_dataset_from_uuid_name(_uuid UUID, _name TEXT, _id_af INT) ;

CREATE OR REPLACE FUNCTION
    gn2pg_flavia.fct_c_get_or_insert_basic_dataset_from_uuid_name(_uuid UUID, _name TEXT, _id_af INT)
    RETURNS INTEGER
AS
$func$
DECLARE
...
COMMENT ON
    FUNCTION gn2pg_flavia.fct_c_get_or_insert_basic_dataset_from_uuid_name(_uuid UUID, _name TEXT, _id_af INT)
...
COMMENT ON FUNCTION gn2pg_flavia.fct_c_get_or_insert_source (_source TEXT)
    IS 'Function to basically create new sources.' ;

-- -------------------------------------------------------------------------------------------------
-- Nomenclatures
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_get_id_nomenclature_from_label(_type TEXT, _label TEXT) ;

CREATE OR REPLACE FUNCTION gn2pg_flavia.fct_c_get_id_nomenclature_from_label(_type TEXT, _label TEXT)
    RETURNS INTEGER
AS
$func$
DECLARE
...
BEGIN
    ...
    FROM ref_nomenclatures.t_nomenclatures AS n
        JOIN ref_nomenclatures.bib_nomenclatures_types AS t
...
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_insert_ds_territories(_id_ds INTEGER, _territories JSONB);

CREATE OR REPLACE
    FUNCTION gn2pg_flavia.fct_c_insert_ds_territories(_id_ds INTEGER, _territories JSONB)
    RETURNS VOID
    LANGUAGE plpgsql
AS
$$
...
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_get_or_create_actors_in_usershub(_actor_role JSONB, _source CHARACTER VARYING) ;

CREATE OR REPLACE
    FUNCTION gn2pg_flavia.fct_c_get_or_create_actors_in_usershub(_actor_role JSONB, _source CHARACTER VARYING)
    RETURNS INTEGER
    LANGUAGE plpgsql
AS
$$
...
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_insert_dataset_actor(_id_dataset INTEGER, _actor_roles JSONB, _source CHARACTER VARYING);

CREATE OR REPLACE
    FUNCTION gn2pg_flavia.fct_c_insert_dataset_actor(_id_dataset INTEGER, _actor_roles JSONB, _source CHARACTER VARYING)
    RETURNS VOID
    LANGUAGE plpgsql
AS
$$
...
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_insert_af_actors(_id_af INTEGER, _actor_roles JSONB, _source CHARACTER VARYING) ;

CREATE OR REPLACE FUNCTION gn2pg_flavia.fct_c_insert_af_actors(_id_af INTEGER, _actor_roles JSONB, _source CHARACTER VARYING) RETURNS VOID
    LANGUAGE plpgsql
AS
$$
DECLARE
...
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_get_or_insert_af_from_af_jsondata(_af_data JSONB, _source CHARACTER VARYING);

CREATE OR REPLACE FUNCTION gn2pg_flavia.fct_c_get_or_insert_af_from_af_jsondata(_af_data JSONB, _source CHARACTER VARYING) RETURNS INTEGER
    LANGUAGE plpgsql
AS
$$
DECLARE
...
COMMENT ON FUNCTION gn2pg_flavia.fct_c_get_or_insert_af_from_af_jsondata(JSONB, VARCHAR)
    IS 'Function to create acquisition framework from json structured data.';

DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_get_or_insert_dataset_from_jsondata(_ds_data JSONB, _id_af INTEGER, _source CHARACTER VARYING);

CREATE OR REPLACE FUNCTION gn2pg_flavia.fct_c_get_or_insert_dataset_from_jsondata(_ds_data JSONB, _id_af INTEGER, _source CHARACTER VARYING) RETURNS INTEGER
    LANGUAGE plpgsql
AS
$$
DECLARE
...
--       , jsonb_build_object('source', _source, 'module', 'gn2pg')
    ...
    WHERE unique_dataset_id = (_ds_data #>> '{uuid}')::UUID;
    PERFORM
        gn2pg_flavia.fct_c_insert_dataset_actor(the_dataset_id, _ds_data -> 'actors', _source);
...
COMMENT ON FUNCTION gn2pg_flavia.fct_c_get_or_insert_dataset_from_jsondata(JSONB, INTEGER, VARCHAR)
    IS 'Function to basically create datasets.';
...
COMMENT ON FUNCTION gn2pg_flavia.fct_c_get_or_insert_source(TEXT)
    IS 'Function to basically create new sources.';
...
CREATE OR REPLACE FUNCTION gn2pg_flavia.fct_tri_c_upsert_data_to_geonature() RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
DECLARE
...
BEGIN
    ...
    SELECT gn2pg_flavia.fct_c_get_or_insert_source(new.source)
    INTO the_id_source;
    --     id_module                                INT;
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_metadata'
                THEN gn2pg_flavia.fct_c_get_or_insert_af_from_af_jsondata(
                    new.item #> '{ca_data}',
                    new.source
                )
            ELSE gn2pg_flavia.fct_c_get_or_insert_basic_af_from_uuid_name(
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_metadata'
                THEN gn2pg_flavia.fct_c_get_or_insert_dataset_from_jsondata(
                    new.item #> '{jdd_data}',
                    the_id_af,
                    new.source
                )
            ELSE gn2pg_flavia.fct_c_get_or_insert_basic_dataset_from_uuid_name(
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('NAT_OBJ_GEO',  new.item #>> '{nature_objet_geo}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('TYP_GRP',  new.item #>> '{type_regroupement}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('METH_OBS',  new.item #>> '{technique_obs}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('STATUT_BIO',  new.item #>> '{statut_biologique}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('ETA_BIO',  new.item #>> '{etat_biologique}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('NATURALITE',  new.item #>> '{naturalite}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('PREUVE_EXIST',  new.item #>> '{preuve_existante}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('STATUT_VALID',  new.item #>> '{statut_validation}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('NIV_PRECIS',  new.item #>> '{precision_diffusion}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('STADE_VIE',  new.item #>> '{stade_vie}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('SEXE',  new.item #>> '{sexe}')
            ELSE ref_nomenclatures.get_id_nomenclature('SEXE',  new.item #>> '{sexe}')
        END AS id_nomenclature_sex
    INTO the_id_nomenclature_sex;
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('OBJ_DENBR',  new.item #>> '{objet_denombrement}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('TYP_DENBR',  new.item #>> '{type_denombrement}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('SENSIBILITE',  new.item #>> '{niveau_sensibilite}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('STATUT_OBS',  new.item #>> '{statut_observation}')
    ...
    SELECT CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('DEE_FLOU',  new.item #>> '{floutage_dee}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('STATUT_SOURCE',  new.item #>> '{statut_source}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('TYP_INF_GEO',  new.item #>> '{type_info_geo}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('OCC_COMPORTEMENT',  new.item #>> '{comportement}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label' THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('STAT_BIOGEO',  new.item #>> '{statut_biogeo}')
    ...
    SELECT
        CASE new.type
            WHEN 'synthese_with_label'
                THEN gn2pg_flavia.fct_c_get_id_nomenclature_from_label('TYPE',  new.item #>> '{label}')
    ...
    SELECT new.item #>> '{validation_date}'
...
COMMENT ON FUNCTION gn2pg_flavia.fct_tri_c_upsert_data_to_geonature()
...
-- DELETE
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_tri_c_delete_data_from_geonature();
...
BEGIN
    ...
    WHERE (old.item #>> '{id_synthese}', gn2pg_flavia.fct_c_get_or_insert_source(old.source)) =
...
COMMENT ON FUNCTION gn2pg_flavia.fct_tri_c_delete_data_from_geonature()
