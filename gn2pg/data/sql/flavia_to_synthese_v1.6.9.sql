/*
 ...
 ⚠️ To update this script compare Gn2Pg version with this URL:
    https://github.com/lpoaura/GN2PG/compare/<old-version>...<new-version>
 Ex.:
    https://github.com/lpoaura/GN2PG/compare/1.6.6...1.6.9
...
-- ⚠️ NOTES: wee need this triggers because dates were not downloaded !
...
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_insert_af_territories (_id_af INTEGER, _territories jsonb) ;
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_insert_af_objectives (_id_af INTEGER, _objectives jsonb) ;
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_insert_af_sinp_theme (_id_af INTEGER, _sinp_themes jsonb) ;
DROP FUNCTION IF EXISTS gn2pg_flavia.fct_c_insert_af_publications (_id_af INTEGER, _objectives jsonb) ;
...
$func$
    ...
    BEGIN
        ...
        THEN
            ...
            IF gn2pg_flavia.fct_c_check_has_additional_data_column(
                'gn_meta',
...
-- -------------------------------------------------------------------------------------------------
-- Acquisition Framework - Territories

CREATE OR REPLACE FUNCTION gn2pg_flavia.fct_c_insert_af_territories(
        _id_af INTEGER,
        _territories jsonb
    )
    RETURNS BOOLEAN
...
-- -------------------------------------------------------------------------------------------------
-- Acquisition Framework - Objectives

CREATE OR REPLACE FUNCTION gn2pg_flavia.fct_c_insert_af_objectives(
        _id_af INTEGER,
        _objectives jsonb
    )
    RETURNS BOOLEAN
...
-- -------------------------------------------------------------------------------------------------
-- Acquisition Framework - Themes

CREATE OR REPLACE FUNCTION gn2pg_flavia.fct_c_insert_af_sinp_theme(
        _id_af INTEGER,
        _sinp_themes jsonb
    )
    RETURNS BOOLEAN
...
$func$
    ...
    BEGIN
        ...
        FOR i IN (SELECT JSONB_ARRAY_ELEMENTS_TEXT(_sinp_themes) item)
            LOOP
                RAISE DEBUG 'itheme % %' , i , i.item ;
                INSERT INTO gn_meta.cor_acquisition_framework_voletsinp (
                    id_acquisition_framework,
                    id_nomenclature_voletsinp
                ) VALUES (
                    _id_af,
                    ref_nomenclatures.get_id_nomenclature('VOLET_SINP', i.item)
                )
                ON CONFLICT DO NOTHING ;
            END LOOP ;
...
-- -------------------------------------------------------------------------------------------------
-- Acquisition Framework - Publications

-- CREATE OR REPLACE FUNCTION gn2pg_flavia.fct_c_insert_af_publications(
--         _id_af INTEGER,
--         _publications jsonb
--     )
--     RETURNS VOID
...
--         i RECORD ;
--     BEGIN
--         RAISE DEBUG '_id_af %, publications %' , _id_af::INT , _publications ;

--         FOR i IN (SELECT JSONB_ARRAY_ELEMENTS_TEXT(_publications) item)
--             LOOP
--                 RAISE DEBUG 'ipublication % %' , i , i.item ;
        ...
        FOR i IN (SELECT jsonb_array_elements_text(_territories) item)
            LOOP
                ...
                INSERT INTO gn_meta.cor_dataset_territory (
                    ...
                    _id_ds,
                    ref_nomenclatures.get_id_nomenclature('TERRITOIRE', coalesce(i.item, 'METROP'))
                )
                ON CONFLICT DO NOTHING ;
            END LOOP ;
    END
        ...
        ELSIF _actor_role ->> 'type_role' = 'role' THEN
            INSERT INTO utilisateurs.t_roles (
                ...
                jsonb_build_object(
                    'source', _source,
                    'module', 'gn2pg',
                    'gn2pg_data', jsonb_build_object('email', _actor_role ->> 'email'))
            )
...
CREATE OR REPLACE
    FUNCTION gn2pg_flavia.fct_c_insert_dataset_actor(
        _id_dataset INTEGER,
        _actor_roles JSONB,
        _source CHARACTER VARYING
    )
...
-- -------------------------------------------------------------------------------------------------
-- Acquisition Framework - Actors

CREATE OR REPLACE FUNCTION gn2pg_flavia.fct_c_insert_af_actors(
        _id_af INTEGER,
        _actor_roles JSONB,
        _source CHARACTER VARYING
    )
...
-- -------------------------------------------------------------------------------------------------
-- Acquisition Framework - Creating from JSON DATA
    ...
    BEGIN
        ...
        THEN
            ...
            SELECT
                ...
                (ref_nomenclatures.get_id_nomenclature(
                    'TYPE_FINANCEMENT',
                    _af_data #>> '{financing_type}'
                ))::INT,
                (ref_nomenclatures.get_id_nomenclature(
                    'NIVEAU_TERRITORIAL',
                    _af_data #>> '{territorial_level}'
                ))::INT,
            ...
            PERFORM gn2pg_flavia.fct_c_insert_af_actors(the_af_id, _af_data -> 'actors', _source) ;
            ...
            PERFORM gn2pg_flavia.fct_c_insert_af_territories(the_af_id, the_territories) ;
            ...
            PERFORM gn2pg_flavia.fct_c_insert_af_objectives(the_af_id, the_objectives) ;
            ...
            PERFORM gn2pg_flavia.fct_c_insert_af_sinp_theme(the_af_id, the_voletsinp) ;
...
-- -------------------------------------------------------------------------------------------------
-- Dataset - Creating from JSON DATA

CREATE OR REPLACE FUNCTION gn2pg_flavia.fct_c_get_or_insert_dataset_from_jsondata(
        _ds_data JSONB,
        _id_af INTEGER,
        _source CHARACTER VARYING
    )
    ...
    BEGIN
        ...
        THEN
            ...
            SELECT
                ...
                (ref_nomenclatures.get_id_nomenclature(
                    'STATUT_SOURCE',
                    _ds_data #>> '{source_status}'
                ))::INT,
                (ref_nomenclatures.get_id_nomenclature(
                    'RESOURCE_TYP',
                    _ds_data #>> '{resource_type}'
                ))::INT,
                (ref_nomenclatures.get_id_nomenclature(
                    'JDD_OBJECTIFS',
                    _ds_data #>> '{dataset_objectif}'
                ))::INT,
                (ref_nomenclatures.get_id_nomenclature(
                    'DS_PUBLIQUE',
                    _ds_data #>> '{data_origin}'
                ))::INT,
                (ref_nomenclatures.get_id_nomenclature(
                    'METHO_RECUEIL',
                    _ds_data #>> '{collecting_method}'
                ))::INT,
                (ref_nomenclatures.get_id_nomenclature(
                    'DATA_TYP',
                    _ds_data #>> '{data_type}'
            ...
            PERFORM gn2pg_flavia.fct_c_insert_dataset_actor(
                the_dataset_id, _ds_data -> 'actors',
                _source
            ) ;

            IF jsonb_array_length(_ds_data -> 'territories') > 0 THEN
                PERFORM gn2pg_flavia.fct_c_insert_ds_territories(
                    the_dataset_id,
                    _ds_data -> 'territories'
                );
            END IF ;
...
$func$
    ...
    BEGIN
        ...
        SELECT
            CASE new.type
                WHEN 'synthese_with_metadata' THEN
                    gn2pg_flavia.fct_c_get_or_insert_af_from_af_jsondata(
                        new.item #> '{ca_data}',
                        new.source
                    )
        ...
        SELECT
            CASE new.type
                WHEN 'synthese_with_metadata' THEN
                    gn2pg_flavia.fct_c_get_or_insert_dataset_from_jsondata(
                        new.item #> '{jdd_data}',
                        the_id_af,
                        new.source
                    )
        ...
        INSERT INTO gn_synthese.synthese (
            ...
            'I'
        ) ON CONFLICT (id_source, entity_source_pk_value) DO UPDATE SET
