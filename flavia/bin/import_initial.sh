#!/bin/bash
# Encoding : UTF-8
# Import in GeoNature Database the FLAVIA Fauna Data for AURA
...
function extractArchive() {
    ...
    if [[ -f "${raw_dir}/${flavia_filename_archive}" ]]; then
        if [[ ! -f "${raw_dir}/${flavia_filename_synthese}" ]]; then
            cd "${raw_dir}/"
            tar jxvf "${raw_dir}/${flavia_filename_archive}"
        else
...
function insertSource() {
    local csv_to_import="${flavia_filename_source%.csv}_rti.csv"
    ...
    if [[ ! -f "${raw_dir}/${csv_to_import}" ]]; then
        ...
        pipenv run python ./bin/gn_import_parser.py \
            ...
            "${raw_dir}/${flavia_filename_source}"
    else
...
function insertOrganism() {
    local csv_to_import="${flavia_filename_organism%.csv}_rti.csv"
    ...
    if [[ ! -f "${raw_dir}/${csv_to_import}" ]]; then
        ...
        pipenv run python ./bin/gn_import_parser.py \
            --type "o" \
            --config "${conf_dir}/parser_actions.ini" \
            "${raw_dir}/${flavia_filename_organism}"
    else
...
function insertUser() {
    local csv_to_import="${flavia_filename_user%.csv}_rti.csv"
...
function insertAcquisitionFramework() {
    local csv_to_import="${flavia_filename_af%.csv}_rti.csv"
...
function insertDataset() {
    local csv_to_import="${flavia_filename_dataset%.csv}_rti.csv"
    ...
    if [[ ! -f "${raw_dir}/${csv_to_import}" ]]; then
        ...
        pipenv run python ./bin/gn_import_parser.py \
            --type "d" \
            --config "${conf_dir}/parser_actions.ini" \
            "${raw_dir}/${flavia_filename_dataset}"
    else
...
function insertSynthese() {
    local csv_to_import="${flavia_filename_synthese%.csv}_rti.csv"
    ...
    if [[ ! -f "${raw_dir}/${csv_to_import}" ]]; then
        ...
        pipenv run python ./bin/gn_import_parser.py \
            --type "s" \
            --config "${conf_dir}/parser_actions.ini" \
            "${raw_dir}/${flavia_filename_synthese}"
    else
