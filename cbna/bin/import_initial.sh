#!/bin/bash
# Encoding : UTF-8
# Import in GeoNature Database the CBNA Flora Data for AURA
...
function insertSource() {
    local csv_to_import="${cbna_filename_source%.csv}_rti.csv"
...
function insertOrganism() {
    local csv_to_import="${cbna_filename_organism%.csv}_rti.csv"
    ...
    if [[ ! -f "${raw_dir}/${csv_to_import}" ]]; then
        ...
        pipenv run python ./bin/gn_import_parser.py \
            --type "o" \
            --config "${conf_dir}/parser_actions.ini" \
            "${raw_dir}/${cbna_filename_organism}"
    else
...
function insertUser() {
    local csv_to_import="${cbna_filename_user%.csv}_rti.csv"
...
function insertAcquisitionFramework() {
    local csv_to_import="${cbna_filename_af%.csv}_rti.csv"
...
function insertDataset() {
    local csv_to_import="${cbna_filename_dataset%.csv}_rti.csv"
...
function insertSynthese() {
    local csv_to_import="${cbna_filename_synthese%.csv}_rti.csv"
    ...
    if [[ ! -f "${raw_dir}/${csv_to_import}" ]]; then
        ...
        pipenv run python ./bin/gn_import_parser.py \
            --type "s" \
            --config "${conf_dir}/parser_actions.ini" \
            "${raw_dir}/${cbna_filename_synthese}"
    else
