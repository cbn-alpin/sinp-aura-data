...
--    * geom not null
--    * "presence" status
--    * "broadcastable" status for diffusion level and sensitivity nomenclatures
...
WITH grand_annecy_areas AS (
    ...
    AND area_code IN (
        '74002', '74004', '74010', '74019', '74036', '74054', '74060', '74061',
        '74062', '74067', '74097', '74108', '74111', '74112', '74137', '74138',
        '74142', '74148', '74176', '74186', '74194', '74198', '74213', '74219',
        '74232', '74233', '74242', '74254', '74267', '74275', '74282', '74299',
        '74303', '74310'
    )
),
grand_annecy_observations AS (
    SELECT
        ...
        CASE
            WHEN a.area_code IS NOT NULL
                THEN jsonb_build_object(
                    'area_code', a.area_code,
                    'type_code', bat.type_code
                )
        END AS area_attachment,
        coalesce(s.meta_update_date, s.meta_create_date) AS derniere_action
    FROM gn_synthese.synthese AS s
        LEFT JOIN gn_exports.synthese_blurred AS sb
            ON sb.id_synthese = s.id_synthese
        JOIN gn_synthese.cor_area_synthese AS cas
            ON cas.id_synthese = s.id_synthese
        JOIN grand_annecy_areas AS gaa
            ON gaa.id_area = cas.id_area
        LEFT JOIN (ref_geo.l_areas AS a
                JOIN ref_geo.bib_areas_types AS bat
                    ON a.id_type = bat.id_type
            )
            ON s.id_area_attachment = a.id_area
    WHERE s.the_geom_4326 IS NOT NULL
        ...
        AND s.id_nomenclature_diffusion_level != ref_nomenclatures.get_id_nomenclature('NIV_PRECIS', '4')
        AND s.id_nomenclature_sensitivity NOT IN (
            ref_nomenclatures.get_id_nomenclature('SENSIBILITE', '4'),
            ref_nomenclatures.get_id_nomenclature('SENSIBILITE', '2.8')
        )
),
af_actors AS (
    SELECT
        ...
        json_build_object(
            ...
            'cd_nomenclature_actor_role', tn.cd_nomenclature,
            'identity', CASE
                WHEN cafa.id_organism IS NOT NULL
                    THEN json_build_object(
                        'organism_name', borg.nom_organisme
                    )
                WHEN cafa.id_role IS NOT NULL
                    THEN json_build_object(
            ...
            END,
            'email', coalesce(borg.email_organisme, tro.email)
        ) AS json_data
    FROM gn_meta.cor_acquisition_framework_actor AS cafa
        LEFT JOIN utilisateurs.bib_organismes AS borg
            ON cafa.id_organism = borg.id_organisme
        LEFT JOIN utilisateurs.t_roles AS tro
...
af_territories AS (
    ...
    FROM gn_meta.cor_acquisition_framework_territory AS caft
...
af_objectives AS (
    ...
    FROM gn_meta.cor_acquisition_framework_objectif AS cafo
        LEFT JOIN ref_nomenclatures.t_nomenclatures
...
af_voletsinp AS (
    ...
    FROM gn_meta.cor_acquisition_framework_voletsinp AS cafv
        LEFT JOIN ref_nomenclatures.t_nomenclatures
...
af_publication AS (
    ...
    FROM gn_meta.cor_acquisition_framework_publication AS cafp
...
af AS (
    SELECT
        ...
        jsonb_build_object(
            ...
            'sinp_theme', af_voletsinp.voletsinp,
    ...
    FROM gn_meta.t_acquisition_frameworks AS taf
        LEFT JOIN gn_meta.t_acquisition_frameworks AS tafp
        ...
        LEFT JOIN ref_nomenclatures.t_nomenclatures AS ntl
            ON taf.id_nomenclature_territorial_level = ntl.id_nomenclature
        LEFT JOIN ref_nomenclatures.t_nomenclatures AS nft
...
ds_protocols AS (
    ...
    FROM gn_meta.cor_dataset_protocol AS cdp
        JOIN gn_meta.sinp_datatype_protocols AS sdp
...
SELECT
    ...
    gao.derniere_action
FROM grand_annecy_observations AS gao
    JOIN gn_synthese.synthese AS s
        ON s.id_synthese = gao.id_synthese
    JOIN ds
