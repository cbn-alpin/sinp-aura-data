-- PNR des volcans d'Auvergne synthese export with geom blurred depending on sensitivity level >= 2015 & < 2020 (2)
...
\echo '----------------------------------------------------------------------------'
\echo 'Create materialized view gn_exports.pnr_volcans_auvergne_blurred_2'

CREATE MATERIALIZED VIEW gn_exports.pnr_volcans_auvergne_blurred_2 AS
WITH
blurred_centroid AS (
	SELECT
		sb.id_synthese,
...
synthese_export AS (
	...
	WHERE s.date_min >= '2015-01-01' AND s.date_min < '2020-01-01'
)
SELECT
    s.id_synthese,
...
CREATE UNIQUE INDEX unique_idx_pnr_volcans_auvergne_blurred_2 ON gn_exports.pnr_volcans_auvergne_blurred_2 (id_synthese);
