-- PNR des volcans d'Auvergne synthese export with geom blurred depending on sensitivity level < 2015 (1)
...
\echo '----------------------------------------------------------------------------'
\echo 'Create materialized view gn_exports.pnr_volcans_auvergne_blurred_1'


CREATE MATERIALIZED VIEW gn_exports.pnr_volcans_auvergne_blurred_1 AS
WITH
blurred_centroid AS (
	SELECT
		sb.id_synthese,
...
CREATE UNIQUE INDEX unique_idx_pnr_volcans_auvergne_blurred_1 ON gn_exports.pnr_volcans_auvergne_blurred_1 (id_synthese);
