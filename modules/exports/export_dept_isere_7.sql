-- export de toutes les données comprises entre 2019 et 2022 sur le département de l'isère
...
\echo '----------------------------------------------------------------------------'
\echo 'Create materialized view gn_exports.dept_isere_7'

CREATE MATERIALIZED VIEW gn_exports.dept_isere_7 AS
WITH
synthese_export AS (
    SELECT DISTINCT
        s.id_synthese,
    ...
    AND s.date_min BETWEEN '2019-01-02' and '2022-01-01'
)
SELECT
    s.id_synthese,
...
CREATE UNIQUE INDEX unique_idx_dept_isere_7
ON gn_exports.dept_isere_7 (id_synthese) ;
