-- export de toutes les données comprises entre 2015 et 2020 sur le département de l'isère
...
\echo '----------------------------------------------------------------------------'
\echo 'Create materialized view gn_exports.dept_isere_5'

CREATE  MATERIALIZED VIEW gn_exports.dept_isere_5 AS
WITH
synthese_export AS (
    SELECT DISTINCT
        s.id_synthese,
    ...
    AND s.date_min BETWEEN '2013-01-02' and '2016-01-01'
)
SELECT
    s.id_synthese,
...
CREATE UNIQUE INDEX unique_idx_dept_isere_5
ON gn_exports.dept_isere_5 (id_synthese) ;
