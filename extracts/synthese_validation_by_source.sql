...
COPY (
    SELECT
        ...
        ) AS code_nomenclature_valid_status,
        COALESCE(s."validator", '\N') AS "validator",
        COALESCE(s.validation_comment, '\N') AS comment,
        gn_commons.determine_auto_validation(
            s."validator", s.meta_validation_date, s.id_nomenclature_valid_status
        ) AS "automatic",
        COALESCE(date_trunc('second', s.meta_validation_date)::text, '\N') AS creation_date,
        '\N' AS additional_data,
    ...
    WHERE s.unique_id_sinp IS NOT NULL
        AND s.id_nomenclature_valid_status IS NOT NULL
