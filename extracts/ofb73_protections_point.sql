...
--end as geom,
st_transform(the_geom_point,2154) as geom,
case 
...
case 
	when st_geometrytype(s.the_geom_local)in ('ST_MultiPoint','ST_Point') then null --pi()*s."precision"*s."precision"
	when st_geometrytype(s.the_geom_local)in ('ST_LineString','ST_MultiLineString') then null --st_area(st_buffer(s.the_geom_local,2,10))
