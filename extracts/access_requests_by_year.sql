...
-- Usage (from local computer): cat ./access_requests_by_year.sql | sed 's/${year}/<my-year>/g'| ssh geonat@db-aura-sinp 'export PGPASSWORD="<db-user-pwd>" ; psql -q -h localhost -p 5432 -U gnreader -d geonature2db' > ./$(date +'%F')_access_requests.csv
